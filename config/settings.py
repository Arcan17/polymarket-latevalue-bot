...
class Settings(BaseSettings):
    ...
    # Filtros de calidad
    dead_zone_pct: float = Field(default=0.001)         # no entrar si spot está a <0.1% del strike
    no_extra_margin: float = Field(default=0.02)        # +2% de edge requerido para lado NO (tie rule)
    max_spread: float = Field(default=0.10)             # máx spread ask-bid para entrar (10c)
    max_vol_30s_pct: float = Field(default=0.003)       # no entrar si BTC movió >0.3% en últimos 30s
    max_entry_price: float = Field(default=1.0)         # no comprar tokens más caros que este precio
    max_edge: float = Field(default=0.35)               # edge >35% = probable libro WS stale → validar con REST

    # Price feeds
    rtds_url: str = Field(default="wss://ws-live-data.polymarket.com")  # Polymarket Chainlink feed
    ...
    # Volatility model
    vol_lookback_seconds: int = Field(default=900)   # segundos de historial RTDS para vol realizada
