...
class PaperExecutor:
    ...
    def enter(self, opp: Opportunity, size_usdc: float = None,
              tte: float = 0.0, vol_30s: float = 0.0,
              book_source: str = "WS", strike_confirmed: bool = False) -> Optional[Position]:
    ...
    def settle(self, pos: Position, spot_price: float, strike: float, direction: str) -> float:
...
class LiveExecutor:
    ...
    def load_persisted_positions(self) -> list[Position]:
        ...
        try:
            ...
            if age_min > 30:
                print(f"[LIVE] Archivo de posiciones tiene {age_min:.0f} min — demasiado viejo, ignorando")
            ...
            print(f"[LIVE] 🔄 Recuperadas {len(recovered)} posición(es) de sesión anterior "
                  f"(guardadas hace {age_min:.0f} min):")
            for p in recovered:
                print(f"[LIVE]   {p.token_side} {p.token_id[:16]}… entrada={p.entry_price:.3f} "
    ...
    def _test_connection(self) -> bool:
        ...
        try:
            ...
            print(f"[LIVE] Conexión OK — latencia API: {latency_ms:.0f}ms "
            ...
            print(f"[LIVE] Conexión HTTP/2 pre-calentada — latencia keep-alive: {latency2_ms:.0f}ms")

            if latency2_ms > 500:
                print(f"[LIVE] ⚠ Latencia alta ({latency2_ms:.0f}ms) — considera migrar a servidor us-east-1")
            elif latency2_ms < 50:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if resp and resp.get("success"):
                    ...
                    fill_price = float(resp.get("price", price)) if resp.get("price") else price
                ...
                if attempt < MAX_RETRIES and any(
                    k in err.lower() for k in ["price", "match", "fill", "fok", "no match"]
                ):
                    ...
                    if new_edge < 0.05:  # si el edge cae a menos del 5% no vale la pena
                        print(f"[LIVE] ✗ FOK rechazado, edge insuficiente con precio ajustado — skip")
                        return None
                    print(f"[LIVE] ⚡ FOK rechazado ({err}) — reintentando @ {price:.3f}")
                    continue
