...
def build_stats(state: dict) -> Panel:
    ...
    table.add_row(
        "Apuestas",
        f"{s.get('bets_placed',0)}",
        "Win rate",
        f"{s.get('win_rate',0):.0%}",
    )
    table.add_row(
        "Ganadas",
        f"[green]{s.get('bets_won',0)}[/]",
        "Perdidas",
        f"[red]{s.get('bets_lost',0)}[/]",
    )
    table.add_row(
        "Apostado",
        f"${s.get('total_wagered',0):.2f}",
        "Mejor edge",
        f"{s.get('best_edge',0):.1%}",
    )
    table.add_row(
        "PnL total",
        f"[{pnl_color}]${pnl:+.4f}[/]",
        "PnL hoy",
        f"[{dpnl_color}]${dpnl:+.4f}[/]",
    )
    table.add_row("ROI", f"[{pnl_color}]{roi:+.1%}[/]", "Capital", f"${cap:.2f}")
...
def build_positions(state: dict) -> Panel:
    ...
    if not positions:
        table.add_row(
            "[dim]—[/]", "[dim]sin posiciones abiertas[/]", "", "", "", "", ""
        )
    else:
        for p in positions:
...
def build_markets(state: dict) -> Panel:
    ...
    def add_section(mlist: list, label: str, color: str) -> None:
        if not mlist:
            return
        table.add_row(
            f"[{color} bold]── {label} ──[/]", "", "", "", "", "", style="on grey19"
        )
...
def build_trades(state: dict) -> Panel:
    ...
    if not trades:
        ...
        for t in reversed(trades):
            ...
            strike_fmt = (
                f"${t['strike']:,.4f}" if t["strike"] < 100 else f"${t['strike']:,.2f}"
            )
